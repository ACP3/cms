...
if (isset($_POST['submit'])) {
	...
	if (isset($errors)) {
		...
		$other_arr = array(
			1 => 'INSERT INTO `{pre}users` VALUES (\'\', \'' . $db->escape($form['user_name']) . '\', \'' . genSaltedPassword($salt, $form['user_pwd']) . ':' . $salt . '\', 1, \'0\', \':1\', \'1:1\', \':1\', \'1\', \'' . $form['mail'] . ':1\', \':1\', \':1\', \':1\', \':1\', \'' . $db->escape($form['date_format_long']) . '\', \'' . $db->escape($form['date_format_short']) . '\', \'' . $form['date_time_zone'] . '\', \'' . $form['date_dst'] .'\', \'' . LANG . '\', \'' . $form['entries'] . '\', \'\')',
