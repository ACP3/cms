...
class SMTP
{
	...
	public function Connect($host, $port = 0, $tval = 30)
	{
		...
		if (empty($this->smtp_conn)) {
			$this->error = array("error" => "Failed to connect to server",
				...
				"errstr" => $errstr);
			if ($this->do_debug >= 1) {
				echo "SMTP -> ERROR: " . $this->error["error"] . ": $errstr ($errno)" . $this->CRLF . $this->br;
			}
			return false;
		}
		...
		if ($this->do_debug >= 2) {
			echo "SMTP -> FROM SERVER:" . $announce . $this->CRLF . $this->br;
		}
	...
	public function StartTLS()
	{
		...
		if ($this->do_debug >= 2) {
			echo "SMTP -> FROM SERVER:" . $rply . $this->CRLF . $this->br;
		}
		...
		if ($code != 220) {
			...
			if ($this->do_debug >= 1) {
				echo "SMTP -> ERROR: " . $this->error["error"] . ": " . $rply . $this->CRLF . $this->br;
			}
			return false;
		}
	...
	public function Quit($close_on_error = true)
	{
		...
		if ($this->do_debug >= 2) {
			echo "SMTP -> FROM SERVER:" . $byemsg . $this->CRLF . $this->br;
		}
		...
		if ($code != 221) {
			...
			$rval = false;
			if ($this->do_debug >= 1) {
				echo "SMTP -> ERROR: " . $e["error"] . ": " . $byemsg . $this->CRLF . $this->br;
			}
		}
	...
	public function Turn()
	{
		$this->error = array("error" => "This method, TURN, of the SMTP " .
			"is not implemented");
		if ($this->do_debug >= 1) {
			echo "SMTP -> NOTICE: " . $this->error["error"] . $this->CRLF . $this->br;
		}
		return false;
	}
	...
	private function get_lines()
	{
		...
		while ($str = @fgets($this->smtp_conn, 515)) {
			if ($this->do_debug >= 4) {
				echo "SMTP -> get_lines(): \$data was \"$data\"" . $this->CRLF . $this->br;
				echo "SMTP -> get_lines(): \$str is \"$str\"" . $this->CRLF . $this->br;
			}
			$data .= $str;
			if ($this->do_debug >= 4) {
				echo "SMTP -> get_lines(): \$data is \"$data\"" . $this->CRLF . $this->br;
			}
