<form action="{uri args="acp/polls/delete"}" method="post">
	<div id="adm-list">
		{check_access mode="link" uri="acp/polls/create" icon="32/kchart" width="32" height="32"}
		{check_access mode="input" action="polls|acp_delete" icon="32/cancel" lang="common|delete_marked"}
...
{/if}
				<td>{$row.period}</td>
				<td>{check_access mode="link" uri="acp/polls/edit/id_`$row.id`" title=$row.question}</td>
				<td>{$row.id}</td>
			</tr>
{/foreach}
		</tbody>
