...
$breadcrumb->append($lang->t('system', 'acp_maintenance'), $uri->route('acp/system/maintenance'))
		   ->append($lang->t('system', 'acp_sql_export'));
...
if (isset($_POST['submit']) === false || isset($errors) === true && is_array($errors) === true) {
	$db_tables = $db->query('SELECT TABLE_NAME FROM information_schema.TABLES WHERE TABLE_TYPE=\'BASE TABLE\' AND TABLE_SCHEMA=\'' . CONFIG_DB_NAME . '\'');
	$tables = array();
	foreach ($db_tables as $row) {
		$table = $row['TABLE_NAME'];
		if (strpos($table, $db->prefix) === 0) {
...
ACP3_View::setContent(ACP3_View::fetchTemplate('system/acp_sql_export.tpl'));