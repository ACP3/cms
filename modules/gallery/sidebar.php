...
$galleries = ACP3_CMS::$db2->fetchAll('SELECT id, start, title FROM ' . DB_PRE . 'gallery WHERE ' . $where . ' ORDER BY start DESC LIMIT ' . $settings['sidebar'], array('time' => ACP3_CMS::$date->getCurrentDateTime()));
$c_galleries = count($galleries);

if ($c_galleries > 0) {
	for ($i = 0; $i < $c_galleries; ++$i) {
		$galleries[$i]['start'] = ACP3_CMS::$date->format($galleries[$i]['start']);
		$galleries[$i]['title_short'] = shortenEntry($galleries[$i]['title'], 30, 5, '...');
	}
