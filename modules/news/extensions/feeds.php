...
$result = $db->select('id, start, headline, text', 'news', '(start = end AND start <= \'' . $date->timestamp() . '\' OR start != end AND start <= \'' . $date->timestamp() . '\' AND end >= \'' . $date->timestamp() . '\')', 'start DESC, end DESC, id DESC', 10);
