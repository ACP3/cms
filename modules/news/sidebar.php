...
$news = ACP3_CMS::$db2->fetchAll('SELECT id, start, title FROM ' . DB_PRE . 'news WHERE ' . $where . ' ORDER BY start DESC, end DESC, id DESC LIMIT ' . $settings['sidebar'], array('time' => ACP3_CMS::$date->getCurrentDateTime()));
$c_news = count($news);
...
if ($c_news > 0) {
	for ($i = 0; $i < $c_news; ++$i) {
		...
		$news[$i]['title_short'] = shortenEntry($news[$i]['title'], 30, 5, '...');
	}
