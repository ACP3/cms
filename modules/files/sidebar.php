...
$files = ACP3_CMS::$db2->fetchAll('SELECT id, start, title FROM ' . DB_PRE . 'files WHERE ' . $where . ' ORDER BY start DESC LIMIT ' . $settings['sidebar'], array('time' => ACP3_CMS::$date->getCurrentDateTime()));
$c_files = count($files);

if ($c_files > 0) {
	for ($i = 0; $i < $c_files; ++$i) {
		$files[$i]['start'] = ACP3_CMS::$date->format($files[$i]['start']);
		$files[$i]['title_short'] = shortenEntry($files[$i]['title'], 30, 5, '...');
	}
