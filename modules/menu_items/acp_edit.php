...
if (ACP3_Validate::isNumber($uri->id) === true && $db->countRows('*', 'menu_items', 'id = \'' . $uri->id . '\'') == 1) {
	...
	if (isset($_POST['submit']) === true) {
		...
		if (isset($errors) === true) {
			...
			setRedirectMessage($bool, $lang->t('common', $bool !== false ? 'edit_success' : 'edit_error'), 'acp/menu_items');
		}
	}
	if (isset($_POST['submit']) === false || isset($errors) === true && is_array($errors) === true) {
		...
		ACP3_View::setContent(ACP3_View::fetchTemplate('menu_items/acp_edit.tpl'));
	}
} else {
