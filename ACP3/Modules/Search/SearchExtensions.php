...
class SearchExtensions {
	...
	public function articlesSearch() {
		...
		$results = Core\Registry::get('Db')->fetchAll('SELECT id, title, text FROM ' . DB_PRE . 'articles WHERE MATCH (' . $fields . ') AGAINST (' . Core\Registry::get('Db')->quote($this->search_term) . ' IN BOOLEAN MODE) AND ' . $period . ' ORDER BY start ' . $this->sort . ', end ' . $this->sort . ', title ' . $this->sort, array('search_term' => $this->search_term, 'time' => Core\Registry::get('Date')->getCurrentDateTime()));
	...
	public function filesSearch() {
		...
		$period = '(start = end AND start <= :time OR start != end AND :time BETWEEN start AND end)';
		$results = Core\Registry::get('Db')->fetchAll('SELECT id, title, text FROM ' . DB_PRE . 'files WHERE MATCH (' . $fields . ') AGAINST (' . Core\Registry::get('Db')->quote($this->search_term) . ' IN BOOLEAN MODE) AND ' . $period . ' ORDER BY start ' . $this->sort . ', end ' . $this->sort . ', id ' . $this->sort, array('time' => Core\Registry::get('Date')->getCurrentDateTime()));
		$c_results = count($results);
		$search_results = array();
	...
	public function newsSearch() {
		...
		$period = '(start = end AND start <= :time OR start != end AND :time BETWEEN start AND end)';
		$results = Core\Registry::get('Db')->fetchAll('SELECT id, title, text FROM ' . DB_PRE . 'news WHERE MATCH (' . $fields . ') AGAINST (' . Core\Registry::get('Db')->quote($this->search_term) . ' IN BOOLEAN MODE) AND ' . $period . ' ORDER BY start ' . $this->sort . ', end ' . $this->sort . ', id ' . $this->sort, array('time' => Core\Registry::get('Date')->getCurrentDateTime()));
		$c_results = count($results);
		$search_results = array();
