...
for ($i = 0; $i < $c_resources; ++$i) {
	if (ACP3\Core\Modules::isActive($resources[$i]['module_name']) === true) {
		$module = ACP3\CMS::$injector['Lang']->t($resources[$i]['module_name'], $resources[$i]['module_name']);
		$output[$module][] = $resources[$i];
	}
}
ksort($output);
ACP3\CMS::$injector['View']->assign('resources', $output);
ACP3\CMS::$injector['View']->assign('can_delete_resource', ACP3\Core\Modules::check('permissions', 'acp_delete_resources'));