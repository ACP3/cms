...
class Model extends Core\Model
{
    ...
    {
        return $this->db->fetchAssoc('SELECT n.*, c.title AS category_title FROM ' . $this->db->getPrefix() . static::TABLE_NAME . ' AS n LEFT JOIN ' . $this->db->getPrefix() . \ACP3\Modules\Categories\Model::TABLE_NAME . ' AS c ON(n.category_id = c.id) WHERE n.id = ?', [$id]);
    }
    ...
    {
        return $this->db->fetchAll('SELECT f.*, c.title AS cat FROM ' . $this->db->getPrefix() . static::TABLE_NAME . ' AS f, ' . $this->db->getPrefix() . \ACP3\Modules\Categories\Model::TABLE_NAME . ' AS c WHERE f.category_id = c.id ORDER BY f.start DESC, f.end DESC, f.id DESC');
    }
