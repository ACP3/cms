    ...
    <nav id="adm-list" class="navbar navbar-default" role="navigation">
        ...
        <div class="collapse navbar-collapse navbar-ex2-collapse">
            <div class="navbar-text pull-right">
                {check_access mode="link" path="acp/articles/index/create" icon="32/contents" width="32" height="32"}
                {check_access mode="input" path="acp/articles/index/delete" icon="32/cancel" lang="system|delete_marked"}
            </div>
        </div>
    </nav>
    {redirect_message}
        ...
        <table id="acp-table" class="table table-striped table-hover">
            ...
            {foreach $articles as $row}
                <tr>
                    ...
                    <td>{check_access mode="link" path="acp/articles/index/edit/id_`$row.id`" title=$row.title}</td>
                    <td>{$row.id}</td>
                </tr>
            {/foreach}
            </tbody>
