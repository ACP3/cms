...
class GuestbookRepository extends Core\Repository\AbstractRepository implements Core\Repository\FloodBarrierAwareRepositoryInterface
{
    ...
    public function getAll(?int $notify = null, ?int $limitStart = null, ?int $resultsPerPage = null): array
    {
        ...
        return $this->db->fetchAll('SELECT IF(g.user_id IS NULL, g.name, u.nickname) AS `name`, IF(g.user_id IS NULL, g.mail, u.mail) AS `mail`, g.id, g.date, g.user_id, g.message FROM ' . $this->getTableName() . ' AS g LEFT JOIN ' . $this->getTableName(\ACP3\Modules\ACP3\Users\Repository\UserRepository::TABLE_NAME) . ' AS u ON(u.id = g.user_id) ' . $where . ' ORDER BY DATE DESC' . $limitStmt);
    }
