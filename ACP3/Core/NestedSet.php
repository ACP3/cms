...
class NestedSet
{
    ...
    public function deleteNode($id)
    {
        $id = (int)$id;
        if (!empty($id)) {
            $lr = $this->db->fetchAssoc('SELECT left_id, right_id FROM ' . $this->tableName . ' WHERE id = ?', [$id]);
            if (!empty($lr)) {
                ...
                try {
                    ...
                    $this->db->executeUpdate('UPDATE ' . $this->tableName . ' SET right_id = right_id - 2 WHERE left_id < ? AND right_id > ?', [$lr['left_id'], $lr['right_id']]);

                    // Nachfolgende Knoten
                    $this->db->executeUpdate('UPDATE ' . $this->tableName . ' SET left_id = left_id - 2, right_id = right_id - 2 WHERE left_id > ?', [$lr['right_id']]);
    ...
    public function insertNode($parentId, array $insertValues)
    {
        ...
        if (empty($parentId) ||
            $this->db->fetchColumn('SELECT COUNT(*) FROM ' . $this->tableName . ' WHERE id = ?', [$parentId]) == 0
        ) {
            $this->db->beginTransaction();
            try {
            ...
            $parent = $this->db->fetchAssoc('SELECT root_id, left_id, right_id FROM ' . $this->tableName . ' WHERE id = ?', [$parentId]);
                    ...
                    if (empty($newParent)) {
                        ...
                        if ($this->enableBlocks === true) {
                            ...
                            if ($items[0]['block_id'] != $blockId) {
                                $newBlock = $this->db->fetchAssoc('SELECT MIN(left_id) AS left_id FROM ' . $this->tableName . ' WHERE block_id = ?', [$blockId]);
    ...
    {
        if ($this->db->fetchColumn('SELECT COUNT(*) FROM ' . $this->tableName . ' WHERE id = ?', [$id]) == 1) {
            ...
            if ($mode === 'up' && $this->db->fetchColumn('SELECT COUNT(*) FROM ' . $this->tableName . ' WHERE right_id = ?' . ($this->enableBlocks === true ? ' AND block_id = ' . $items[0]['block_id'] : ''), [$items[0]['left_id'] - 1]) > 0) {
            ...
            } elseif ($mode === 'down' && $this->db->fetchColumn('SELECT COUNT(*) FROM ' . $this->tableName . ' WHERE left_id = ?' . ($this->enableBlocks === true ? ' AND block_id = ' . $items[0]['block_id'] : ''), [$items[0]['right_id'] + 1]) > 0) {
