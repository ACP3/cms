...
if (validate::isNumber($uri->id) && $db->select('id', 'pages_blocks', 'id = \'' . $uri->id . '\'', 0, 0, 0, 1) == '1') {
	...
	if (isset($_POST['submit'])) {
		...
		if (preg_match('/^[a-zA-Z]+\w/', $form['index_name']) && $db->select('id', 'pages_blocks', 'index_name = \'' . $db->escape($form['index_name']) . '\' AND id != \'' . $uri->id . '\'', 0, 0, 0, 1) > 0)
