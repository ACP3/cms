...
<form action="{$REQUEST_URI}" method="post" accept-charset="UTF-8">
	<fieldset>
		...
		<dl>
			<dt><label for="width">{lang t="emoticons|image_width"}</label></dt>
			<dd><input type="number" name="form[width]" id="width" value="{$form.width}"></dd>
			<dt><label for="height">{lang t="emoticons|image_height"}</label></dt>
			<dd><input type="number" name="form[height]" id="height" value="{$form.height}"></dd>
			<dt><label for="filesize">{lang t="emoticons|image_filesize"}</label></dt>
