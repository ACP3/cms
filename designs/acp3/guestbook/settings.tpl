...
{/foreach}
			...
			<dd>
				<select name="form[notify]" id="notify">
					<option value="">{lang t="common|pls_select"}</option>
...
{foreach $newsletter_integration as $row}
				<label for="newsletter-integration-{$row.value}">
					<input type="radio" name="form[newsletter_integration]" id="newsletter-integration-{$row.value}" value="{$row.value}" class="checkbox"{$row.checked}>
					{$row.lang}
				</label>
{/foreach}
			</dd>
