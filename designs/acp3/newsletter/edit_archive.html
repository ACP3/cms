...
<form action="{$REQUEST_URI}" method="post" accept-charset="UTF-8">
	<fieldset>
		...
		<dl>
			...
			<dd><input type="text" name="form[subject]" id="subject" value="{$form.subject}"></dd>
			<dt><label for="text">{lang t="newsletter|text"}</label></dt>
			<dd><textarea name="form[text]" id="text" cols="50" rows="5">{$form.text}</textarea></dd>
			<dt><label for="test-1">{lang t="newsletter|test_newsletter"}</label> <span>({lang t="newsletter|test_nl_description"})</span></dt>
			<dd>
{foreach $test as $row}
