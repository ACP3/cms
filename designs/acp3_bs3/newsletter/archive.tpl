{if isset($newsletters)}
	<script type="text/javascript">
		$(document).ready(function() {
			$('#newsletter-submit').hide();
			$('#newsletters').on('change', function() {
				$(this).closest('form').submit();
			});
		});
	</script>
	<div class="navbar navbar-default">
		<form action="{$REQUEST_URI}" method="post" class="navbar-form navbar-right">
			<div class="form-group">
				<select class="form-control" id="newsletters" name="newsletter">
					<option value="">{lang t="system|pls_select"}</option>
					{foreach $newsletters as $row}
						<option value="{$row.id}"{$row.selected}>{$row.title} - {$row.date_formatted}</option>
					{/foreach}
				</select>
			</div>
			<button type="submit" name="categories" id="newsletter-submit" class="btn btn-primary">{lang t="system|submit"}</button>
		</form>
	</div>
	{if isset($newsletter)}
		<article class="dataset-box">
			<header class="navbar navbar-default">
				<div class="navbar-header">
					<h2 class="navbar-brand">{$newsletter.title}</h2>
				</div>
				<small class="navbar-text pull-right">
					<time datetime="{$newsletter.date_iso}">{$newsletter.date_formatted}</time>
				</small>
			</header>
			<div class="content">
				{$newsletter.text}
			</div>
		</article>
	{else}
		<div class="alert alert-warning text-center">
			<strong>{lang t="newsletter|select_newsletter"}</strong>
		</div>
	{/if}
{else}
	<div class="alert alert-warning text-center">
		<strong>{lang t="system|no_entries"}</strong>
	</div>
{/if}