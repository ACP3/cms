...
if (isset($_POST['update'])) {
	...
	if (CONFIG_DB_VERSION < 1) {
		...
		$results[] = executeSqlQueries($queries, 1);
	}
	if (CONFIG_DB_VERSION < 2) {
		...
		$results[] = executeSqlQueries($queries, 2);
	}
	if (CONFIG_DB_VERSION < 3) {
		...
		$results[] = executeSqlQueries($queries, 3);
	}
	if (CONFIG_DB_VERSION < 4) {
		...
		$results[] = executeSqlQueries($queries, 4);

		$user = ACP3_CMS::$db2->fetchColumn('SELECT MIN(id) AS id FROM ' . DB_PRE . 'users');

		ACP3_CMS::$db2->executeUpdate('UPDATE ' . DB_PRE . 'files SET user_id = ?', array($user));
		ACP3_CMS::$db2->executeUpdate('UPDATE ' . DB_PRE . 'gallery SET user_id = ?', array($user));
		ACP3_CMS::$db2->executeUpdate('UPDATE ' . DB_PRE . 'news SET user_id = ?', array($user));
		ACP3_CMS::$db2->executeUpdate('UPDATE ' . DB_PRE . 'newsletter_archive SET user_id = ?', array($user));
		ACP3_CMS::$db2->executeUpdate('UPDATE ' . DB_PRE . 'polls SET user_id = ?', array($user));
		ACP3_CMS::$db2->executeUpdate('UPDATE ' . DB_PRE . 'static_pages SET user_id = ?', array($user));
	}
	if (CONFIG_DB_VERSION < 5) {
		...
		$results[] = executeSqlQueries($queries, 5);

		$dir = scandir(MODULES_DIR);
		foreach ($dir as $row) {
			if ($row !== '.' && $row !== '..' && is_file(MODULES_DIR . $row . '/module.xml') === true) {
				ACP3_CMS::$db2->insert(DB_PRE . 'modules', array('name' => $row, 'active' => 1));
			}
		}
	}
	if (CONFIG_DB_VERSION < 10) {
		...
		$results[] = executeSqlQueries($queries, 10);

		$roles = ACP3_CMS::$db2->fetchAll('SELECT id, left_id, right_id FROM ' . DB_PRE . 'acl_roles');
		foreach ($roles as $row) {
			$parent_id = ACP3_CMS::$db2->fetchColumn('SELECT id FROM ' . DB_PRE . 'acl_roles WHERE left_id < ? AND right_id > ? ORDER BY left_id DESC LIMIT 1', array($row['left_id'], $row['right_id']));
			ACP3_CMS::$db2->update(DB_PRE . 'acl_roles', array('parent_id' => !empty($parent_id) ? $parent_id : 0), array('id' => $row['id']));
		}

		$pages = ACP3_CMS::$db2->fetchAll('SELECT id, left_id, right_id FROM ' . DB_PRE . 'menu_items');
		foreach ($pages as $row) {
			$parent_id = ACP3_CMS::$db2->fetchColumn('SELECT id FROM ' . DB_PRE . 'menu_items WHERE left_id < ? AND right_id > ? ORDER BY left_id DESC LIMIT 1', array($row['left_id'], $row['right_id']));
			ACP3_CMS::$db2->update(DB_PRE . 'menu_items', array('parent_id' => !empty($parent_id) ? $parent_id : 0), array('id' => $row['id']));
		}
	...
	if (CONFIG_DB_VERSION < 17) {
		...
		$results[] = executeSqlQueries($queries, 17);
	}
	...
	if (CONFIG_DB_VERSION < 21) {
		...
		$results[] = executeSqlQueries($queries, 21);
	}
	if (CONFIG_DB_VERSION < 22) {
		$mod_id = ACP3_CMS::$db2->fetchColumn('SELECT id FROM ' . DB_PRE . 'modules WHERE name = ?', array('access'));
		$queries = array(
			"INSERT INTO `{pre}acl_resources` (`id`, `module_id`, `page`, `params`, `privilege_id`) VALUES ('', '" . $mod_id . "', 'create_resources', '', 4);",
		);
		$results[] = executeSqlQueries($queries, 22);
	}
	if (CONFIG_DB_VERSION < 23) {
		...
		$results[] = executeSqlQueries($queries, 23);
	}
	...
	if (CONFIG_DB_VERSION < 25) {
		...
		$results[] = executeSqlQueries($queries, 25);
	}
	...
	if (CONFIG_DB_VERSION < 27) {
		...
		$results[] = executeSqlQueries($queries, 27);
	}
	...
	if (CONFIG_DB_VERSION < 29) {
		...
		$results[] = executeSqlQueries($queries, 29);
	}
	if (CONFIG_DB_VERSION < 30) {
		...
		$mod_id = ACP3_CMS::$db2->fetchColumn('SELECT id FROM ' . DB_PRE . 'modules WHERE name = ?', array('system'));
		foreach ($system_settings as $key => $value) {
			ACP3_CMS::$db2->insert(DB_PRE . 'settings', array('id' => '', 'module_id' => $mod_id, 'name' => $key, 'value' => $value));
		}
		...
		$results[] = executeSqlQueries($queries, 30);
	}
