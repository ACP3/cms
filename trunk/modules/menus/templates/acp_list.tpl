...
{if $can_edit}
						<a href="{uri args="acp/menus/edit/id_`$values.menu_id`"}" class="btn btn-small" title="{lang t="menus|acp_edit"}"><i class="icon-edit"></i> {lang t="system|edit"}</a>
{/if}
{if $can_delete}
						<a href="{uri args="acp/menus/delete/entries_`$values.menu_id`"}" class="btn btn-small" title="{lang t="menus|acp_delete"}"><i class="icon-remove"></i> {lang t="system|delete"}</a>
{/if}
					</div>
{/if}
				</td>
