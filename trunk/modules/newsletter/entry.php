...
switch ($modules->action) {
	...
	case 'delete':
		...
		if (is_array($entries)) {
			...
			$content = combo_box(lang('newsletter', 'confirm_delete'), uri('acp/newsletter/adm_list/action_delete/entries_' . $marked_entries), uri('acp/newsletter'));
		} elseif (ereg('^([0-9|]+)$', $entries) && isset($modules->gen['confirmed'])) {
			$marked_entries = explode('|', $entries);
			$bool = 0;
			foreach ($marked_entries as $entry) {
				if (!empty($entry) && ereg('[0-9]', $entry) && $db->select('id', 'nl_accounts', 'id = \'' . $entry . '\'', 0, 0, 0, 1) == '1')
