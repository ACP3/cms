...
if ($c_accounts > 0) {
	...
	for ($i = 0; $i < $c_accounts; ++$i) {
		if (!empty($accounts[$i]['hash'])) {
			$accounts[$i]['status'] = '<a href="' . uri('acp/newsletter/adm_activate/id_' . $accounts[$i]['id']) . '" title="' . $lang->t('newsletter', 'activate_account') . '"><img src="' . ROOT_DIR . 'images/crystal/16/cancel.png" alt="" /></a>';
		} else {
