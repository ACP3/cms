...
if (ACP3_Validate::isNumber($uri->id) === true && $db->countRows('*', 'gallery', 'id = \'' . $uri->id . '\'') == 1) {
	...
	if (isset($_POST['submit']) === true) {
		...
		if (isset($errors) === true) {
			...
			setRedirectMessage($bool, $lang->t('common', $bool !== false ? 'edit_success' : 'edit_error'), 'acp/gallery');
		}
	}
	if (isset($_POST['entries']) === false && isset($_POST['submit']) === false || isset($errors) === true && is_array($errors) === true) {
		...
		if ($c_pictures > 0) {
			...
			$tpl->assign('can_delete', ACP3_Modules::check('gallery', 'acp_delete_picture'));
			$tpl->assign('can_order', ACP3_Modules::check('gallery', 'acp_order'));
			$tpl->assign('can_edit_picture', ACP3_Modules::check('gallery', 'acp_edit_picture'));
		}
