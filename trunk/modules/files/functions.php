...
{
	$data = ACP3_CMS::$db2->fetchAssoc('SELECT f.id, f.start, f.category_id, f.file, f.size, f.title, f.text, f.comments, c.title AS category_name FROM ' . DB_PRE . 'files AS f, ' . DB_PRE . 'categories AS c WHERE f.id = ? AND f.category_id = c.id', array($id));
