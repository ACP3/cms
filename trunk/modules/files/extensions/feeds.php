...
$result = ACP3_CMS::$db->select('id, start, link_title, text', 'files', '(start = end AND start <= \'' . $time . '\' OR start != end AND start <= \'' . $time . '\' AND end >= \'' . $time . '\')', 'start DESC, end DESC, id DESC', 10);
$c_result = count($result);

for ($i = 0; $i < $c_result; ++$i) {
	$item = $feed->createNewItem();
	$item->setTitle(ACP3_CMS::$db->escape($result[$i]['link_title'], 3));
