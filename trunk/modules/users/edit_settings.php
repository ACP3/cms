...
if (ACP3_CMS::$auth->isUser() === false || ACP3_Validate::isNumber(ACP3_CMS::$auth->getUserId()) === false) {
	...
	if (isset($_POST['submit']) === false || isset($errors) === true && is_array($errors) === true) {
		$user = ACP3_CMS::$db2->fetchAssoc('SELECT date_format_long, date_format_short, time_zone, language, entries FROM ' . DB_PRE . 'users WHERE id = ?', array(ACP3_CMS::$auth->getUserId()));
