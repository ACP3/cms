...
if (validate::isNumber($uri->id) && $db->select('id', 'poll_question', 'id = \'' . $uri->id . '\' AND start <= \'' . $date->timestamp() . '\'', 0, 0, 0, 1) == 1) {
	...
	$total_votes = $db->select('answer_id', 'poll_votes', 'poll_id = \'' . $uri->id . '\'', 0, 0, 0, 1);

	for ($i = 0; $i < $c_answers; ++$i) {
		$answers[$i]['text'] = $answers[$i]['text'];
		$answers[$i]['votes'] = $db->select('answer_id', 'poll_votes', 'poll_id = \'' . $uri->id . '\' AND answer_id = \'' . $answers[$i]['id'] . '\'', 0, 0, 0, 1);
