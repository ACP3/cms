...
if (validate::isNumber($uri->id) && $db->countRows('*', 'poll_question', 'id = \'' . $uri->id . '\' AND start <= \'' . $date->timestamp() . '\'') == 1) {
	...
	$total_votes = $db->select('COUNT(answer_id)', 'poll_votes', 'poll_id = \'' . $uri->id . '\'');

	for ($i = 0; $i < $c_answers; ++$i) {
		$answers[$i]['text'] = $answers[$i]['text'];
		$answers[$i]['votes'] = $db->select('COUNT(answer_id)', 'poll_votes', 'poll_id = \'' . $uri->id . '\' AND answer_id = \'' . $answers[$i]['id'] . '\'');
