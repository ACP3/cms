		...
		if (self::isUriSafe($alias)) {
			if (is_dir(MODULES_DIR . $alias) === true) {
				...
				if ($path !== '/' && ACP3_Validate::isInternalURI($path) === true) {
					return ACP3_CMS::$db2->fetchColumn('SELECT COUNT(*) FROM ' . DB_PRE . 'seo WHERE alias = ? AND uri != ?', array($alias, $path)) > 0 ? true : false;
				} elseif (ACP3_CMS::$db2->fetchColumn('SELECT COUNT(*) FROM ' . DB_PRE . 'seo WHERE alias = ?', array($alias)) > 0) {
					return true;
				}
			}
		}
