...
function commentsList($module, $entry_id)
{
	...
	if ($c_comments > 0) {
		...
		if ($settings['emoticons'] == 1) {
			$emoticons_active = ACP3\Core\Modules::check('emoticons', 'functions') === true ? true : false;
			if ($emoticons_active === true) {
				require_once MODULES_DIR . 'emoticons/functions.php';
			}
		}

		ACP3\CMS::$injector['View']->assign('pagination', ACP3\Core\Functions::pagination(commentsCount($module, $entry_id)));
	...
	if (ACP3\Core\Modules::check('comments', 'create') === true) {
