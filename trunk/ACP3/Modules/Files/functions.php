...
function setFilesCache($id)
{
	...
	return ACP3\Core\Cache::create('details_id_' . $id, $data, 'files');
}
/**
...
{
	if (ACP3\Core\Cache::check('details_id_' . $id, 'files') === false)
		setFilesCache($id);

	return ACP3\Core\Cache::output('details_id_' . $id, 'files');
}