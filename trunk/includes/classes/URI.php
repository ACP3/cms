...
class ACP3_URI
{
	...
	function __construct($defaultModule = '', $defaultFile = '')
	{
		...
		if ((bool) preg_match('=includes/min/index\.php$=', $_SERVER['PHP_SELF']) === false) {
	...
	private function checkForUriAlias()
	{
		...
		if ((bool) (bool) CONFIG_SEO_ALIASES === true && !defined('IN_ADM')) {
			...
			$alias = ACP3_CMS::$db2->fetchAssoc('SELECT uri FROM ' . DB_PRE . 'seo WHERE alias = ?', array(substr($this->query, 0, -1)));
			if (!empty($alias)) {
