...
class ACP3_Modules
{
	...
	{
		return ACP3_CMS::$db2->fetchColumn('SELECT COUNT(*) FROM ' . DB_PRE . 'modules WHERE name = ?', array($module)) == 1 ? true : false;
	}
	/**
	...
	public static function setModulesCache()
	{
		...
		foreach ($dirs as $dir) {
			...
			if ($dir !== '.' && $dir !== '..' && is_file($path) === true) {
				$mod_info = ACP3_XML::parseXmlFile($path, 'info');

				if (!empty($mod_info)) {
					$mod_db = ACP3_CMS::$db2->fetchAssoc('SELECT version, active FROM ' . DB_PRE . 'modules WHERE name = ?', array($dir));
					$infos[$dir] = array(
						'dir' => $dir,
					...
					$infos[$dir]['dependencies'] = array_values(ACP3_XML::parseXmlFile($path, 'info/dependencies'));
				}
			}
		}
